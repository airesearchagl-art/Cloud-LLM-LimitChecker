"""Read/write access to the local Codex usage manual-snapshot cache file.

Codex CLI (as of the version checked) has no official machine-readable route
(CLI flag, hook, or API) for the ChatGPT-plan Codex usage limits (5-hour /
weekly windows) — only interactive display (`/status`, the Usage web page) is
available. This module stores a value the user manually read off one of those
official screens and typed into this app; it is never populated by running
Codex, scraping `/status`, or calling any Codex API.

The cache file lives outside this repository and outside `~/.codex/`, so it
is clearly separated from both the checked-out project and Codex's own state.
"""
...
SCHEMA_VERSION = 1
SOURCE_NAME = "codex_manual"

# Manual confirmation, not a push/poll feed: staleness here means "the user
# hasn't re-checked /status or the Usage page in a while", not "fetch failed".
# Deliberately a much longer window than Claude Code's push-based statusLine
# cache (15 minutes), since re-checking Codex usage is a manual chore.
...
_CACHE_FILE_NAME = "codex-usage.json"
...
def _validate_window(raw: object) -> dict | None:
    """Validate one usage window (`five_hour` / `weekly`). `None` is a valid, expected value."""
    if raw is None:
        return None
...
def validate_cache_record(record: object, *, now: datetime) -> dict:
    ...
    Used both before writing (by the `PUT /api/codex-usage` handler) and
    before reading (`load_snapshot` below), so a record can only ever reach
    display or disk after passing the exact same checks. Raises
    `CacheValidationError` (never anything else) on any violation; returns a
    normalized dict — aware UTC ISO timestamps, floats — on success. Never
    includes any part of the raw input in the exception message.
    """
...
def write_cache_atomic(record: dict, path: Path) -> None:
    ...
    fd, tmp_name = tempfile.mkstemp(dir=str(path.parent), prefix=".tmp-codex-usage-", suffix=".json")
    try:
